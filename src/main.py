...
for index, row in df.iterrows():
    ...
    with open(f'834_{index}.X12', 'w') as f:
        ...
        f.write(
            ...
            '*030101*1253*U*00401*000000001*0*P*>\n'
        )
        ...
        f.write(
            f'INS*{row["RelationshipCode"]}*{row["MemberActionCode"]}**'
            f'{row["MaintenanceTypeCode"]}**{row["MaintenanceReasonCode"]}**'
            f'{row["BenefitStatusCode"]}**{row["MedicareStatusCode"]}**'
            f'{row["COBRACode"]}**{row["EmploymentStatusCode"]}**'
            f'{row["StudentStatusCode"]}**{row["HandicapIndicator"]}**'
            f'{row["DateOfBirth"]}\n'
        )
        ...
        f.write(
            f'NM1*IL*1*{row["LastName"]}*{row["FirstName"]}**{row["MiddleName"]}**'
            f'{row["NamePrefix"]}**{row["NameSuffix"]}\n'
        )
        ...
        f.write(
            f'DMG*D8*{row["DateOfBirth"]}*{row["GenderCode"]}**{row["MaritalStatusCode"]}**'
            f'{row["RaceOrEthnicityCode"]}**{row["CitizenshipStatusCode"]}**{row["CountryCode"]}\n'
        )
